...
async function loadProject() {
  ...
  try {
    ...
    // =====================================================================================
    // 📌 SEO OPTIMIZATION BLOCK (TAMBAHAN PENTING AGAR BISA INDEX GOOGLE)
    // =====================================================================================
    ...
    const ld = {
      ...
      "author": {
        "@type": "Person",
        "name": "Den Mardiyana Saputra"
      }
    };

    let scriptLD = document.getElementById("structured-data");
    if (!scriptLD) {
      scriptLD = document.createElement("script");
      scriptLD.id = "structured-data";
    ...
    if (aboutText)
      aboutText.textContent = project.about || project.description || "";
    ...
    if (showcase) {
      showcase.innerHTML = (project.screenshots || [project.preview || project.img])
        .map(
          img =>
            `<img src="${img}" alt="${project.title || project.name}" class="rounded-xl shadow-lg hover:scale-105 transition">`
        )
        .join("");
    }
